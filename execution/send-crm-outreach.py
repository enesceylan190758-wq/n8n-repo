...
def send_one(
    ...
    default_from = os.environ.get("BLOG_SMTP_FROM", user)
    # Aynı domain alias (enes@ / abdulkadir@) Birincil'e daha yakın; Hostinger genelde kabul eder
...
def run_payload(payload: dict) -> dict:
    ...
    if mail_style in ("marka", "card"):
        mail_style = "branded"
    # Kişisel stilde From = gönderen maili (enes@ / abdulkadir@)
    from_email = ""
    if mail_style == "personal":
    ...
    return {
        ...
        "from": os.environ.get("BLOG_SMTP_FROM", "info@nefalix.com"),
        "replyTo": reply_to,
