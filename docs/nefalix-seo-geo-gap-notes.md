...
1. **Faz 0 teknik:** `/geo` ve `/blog` indeks SSR — **yapıldı** (landing `blog-index` / `geo-index`).
2. **Faz 1:** Sağlık keyword + rakip haritasını `geo-topics` / content brief’e bağla (otel/oto sonra).
3. **Faz 2–3:** Sağlık GEO + karşılaştırma sayfaları — hukuk onayı sonrası; mevcut günlük GEO’yu silip yeniden yazma, üstüne stratejik sayfalar.
4. **Faz 5 ölçüm:** `docs/geo-prompt-baseline.md` + GSC; plan §9 metrikleri.

Director master prompt: plan §6 — yeni içerik turunda `directives/` altına SOP olarak taşınabilir.
