...
1. Vertex Gemini ile 1 Türkçe **GEO formatlı** blog yazısı (answer-first + H2 + SSS)
2. Kapak + alt görsel
3. Supabase `blog_posts` kaydı → sitede yayın
4. `info@nefalix.com` ile yöneticilere mail

Aynı sabah **09:15** günlük GEO paketi için bkz. `directives/geo.md`.
...
| `Unterminated string` / JSON parse | `BLOG_RESPONSE_SCHEMA` + `parse_json_relaxed` + 5 retry; log: `/var/log/nefalix-blog.log` |
...
| YouTube embed yok | `youtube_video_id` + `pick_youtube_video`; backfill: `execution/backfill-youtube-ids.py` |
